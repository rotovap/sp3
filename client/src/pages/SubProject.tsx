...
import {
  GetPathToProjectHandlerResponse,
  GetProjectByIdHandlerResponse,
  ...
  useEffect(() => {
    const apiCall = async () => {
      ...
      const responsePath = await fetch(
        `http://localhost:3000/projects/pathToProject/${id}`,
      );
      const resultProject: GetProjectByIdHandlerResponse =
        await responseProject.json();
      const resultPath: GetPathToProjectHandlerResponse =
        await responsePath.json();
