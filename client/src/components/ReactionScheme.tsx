...
import { ExperimentWithReagents, ReagentWithSMILES } from "../../../server/routes/experiments"
                    ...
                    <Stack spacing={3}>
                        <Stack spacing={0}>
                            {aboveArrowReagents.map((i) => {
                                return (
                                    <Stack key={i.reagentId} marginLeft={REACTION_ARROW_MARGIN_LEFT}>
                                        <Typography variant="caption" display="block" gutterBottom>
                                            {i.reagent.name}
                                        </Typography>
                                    </Stack>
                            ...
                            <div key={i.reagentId}>
                                {idx === 0 ?
                                    <ArcherElement id={`right-side-0`}>
                                        <div style={boxStyle}>
                                            <LeftAndRightSide idx={i.reagentId} reagent={i.reagent} lengthOfReagents={rightSideReagents.length} />
                                        </div>
                                    </ArcherElement>
                                    :
